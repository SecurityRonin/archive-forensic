//! The unified **disk-image archive layer** entry point. A single function the
...
//!
//! Unlike [`crate::resolve`] (content-driven, unwraps everything), this is for a
//! single wrapped *evidence image*: it peels one bare gzip/bzip2 wrapper —
...
//! [`Peel::NotPacked`] and left to the caller. Each consumer keeps its own
...
/// image) return [`Peel::NotPacked`], as does anything unrecognized.
///
...
/// [`Peel::NotPacked`] (the coincidental-magic guard). Archive extraction is
...
/// Enforce the archive layer's cumulative inflated cap on a single extraction.
