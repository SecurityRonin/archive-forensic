//! The unified disk-image archive layer: `peel_archive` peels a bare gz/bz2
//! wrapper OR extracts the single member of a one-member archive to inner bytes,
//! and leaves a multi-member archive (a collection) as `NotPacked`. This is the
//! entry point the disk-forensic / 4n6mount consumers relocate onto.
#![allow(clippy::unwrap_used, clippy::expect_used)]

use archive_core::{peel_archive, Limits, Peel};
...
fn peels_bare_bzip2_wrapper() {
    ...
    .unwrap()
    {
        Peel::Inner(inner) => assert_eq!(inner, expected),
        Peel::NotPacked => panic!("bare bzip2 with .bz2 name must peel"),
    }
}
...
fn coincidental_magic_without_extension_is_not_packed() {
    ...
    match peel_archive(&raw, Some("disk.raw"), &Limits::default()).unwrap() {
        Peel::NotPacked => {}
        Peel::Inner(_) => panic!("must NOT peel a gzip-magic file named .raw"),
    }
}
...
fn extracts_single_member_archive() {
    ...
    match peel_archive(&load("oneimg.tgz"), Some("oneimg.tgz"), &Limits::default()).unwrap() {
        Peel::Inner(inner) => assert_eq!(
            inner,
            b"RAW-EVIDENCE-IMAGE-BYTES payload for the single-member detour\n"
        ),
        Peel::NotPacked => panic!("single-member archive must extract"),
    }
}
...
fn multi_member_archive_is_not_packed() {
    ...
    {
        Peel::NotPacked => {}
        Peel::Inner(_) => panic!("multi-member archive must be left to the caller"),
    }
}
...
fn plain_raw_is_not_packed() {
    ...
    assert!(matches!(
        peel_archive(raw, Some("disk.dd"), &Limits::default()).unwrap(),
        Peel::NotPacked
    ));
}
...
fn over_cap_single_extract_fails_loud() {
    ...
    assert!(peel_archive(&load("oneimg.tgz"), Some("oneimg.tgz"), &limits).is_err());
}
